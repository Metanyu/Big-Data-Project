...
def send_batch(session, url, records):
    ...
    try:
        # Bulk serialize with datetime handling
        payload = [
    ...
    try:
        for row in iterator:
            ...
            if time_delta > 0:
                ...
                if sleep_debt > 0.01:
                    # Flush pending data BEFORE sleeping
                    # Check if enough time passed OR if we are forced to sleep
                    # We flush here to ensure data is sent before we pause
                    if pending_batch:
            ...
            # 4. "Bus" Batch Flush (Time Based)
            # If we haven't slept recently but data is piling up, flush periodically
            # e.g. every 1 second real-time
