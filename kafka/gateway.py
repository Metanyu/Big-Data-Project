...
class TripEvent(BaseModel):
    # Accept any dict structure since we are just forwarding the JSON
    # This avoids strict validation overhead for high-throughput
    data: Dict[str, Any]
...
async def ingest_trip(event: Dict[str, Any]):
    ...
    try:
        # Use VendorID as partition key to keep same taxi's data ordered
        key = str(event.get('VendorID', '')).encode('utf-8')
        value = json.dumps(event).encode('utf-8')
        
        # Async produce (fire and forget for speed, relying on delivery reports in bg)
...
async def ingest_trips(events: list[Dict[str, Any]]):
    ...
    try:
        ...
        for event in events:
            ...
            count += 1
            if count % 1000 == 0:
                producer.poll(0) # Serve delivery reports occasionally
        
        producer.poll(0) # Final poll
